...
class NonLocalBlock(nn.Module):
    """
    reference: Non-local block  Non-local Neural Networks
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        # self.block1 = NonLocalBlock(512 * block.expansion)
        # self.block2 = NonLocalBlock(512 * block.expansion)
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        # print(x.size())#torch.Size([32, 2048, 7, 7])
        # x = self.block1(x)
        # x = self.block2(x)
        x = self.avgpool(x)
        # print(x.size())# torch.Size([32, 2048, 1, 1])
        x = torch.flatten(x, 1)
        x = self.fc(x)
